...
bool decompose_time(int counter, char type);
    ...
    while (true)
    {
    	for(int i = 0; i < BALLS; i++)
    	{
    		if (ballen[i].exists == true)
    		{
				...
				if (decompose_time(ballen[i].counter, ballen[i].type))
				{
					decompose(ballen, i);
				}
...
void initDataStructure(node* hastable)
{
	...
	char* colors[10] = {"BLUE", "RED", "CYAN", "ORANGE", "YELLOW", "GRAY", "GREEN", "MAGENTA", "PINK", "DARK_GRAY"};
	char types[10] = {'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J'};
	hastable = calloc(10, sizeof(node));
	node* first = hastable;
	for(int i = 0; i < 10; i++)
	{
		// make blueprint for ball type 
...
void collision(BALL* ball1, BALL* ball2)
{
	...
	if (React && drand48() < chance[reaction_type])
	{
